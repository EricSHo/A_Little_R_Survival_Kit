...
Here we visualize the trend between `Sepal.Width` and `Sepal.Length` (2^nd^ row and 1^st^ column), and `Sepal.Width` and `Petal.Length` (2^nd^ row and 3^rd^ column). Despite the trend not being clear between `Sepal.Width` and `Petal.Length` , let's proceed to build an MLR model.

```{r}
mlr_model <- lm(Sepal.Width ~ Sepal.Length + Petal.Length, data=setosa_iris)
summary(mlr_model)
```

Based on the p-values, only `Sepal.Length` is a stable predictor of `Sepal.Width`.
