...
The `subset()` function selects rows from a data frame if they satisfy the condition(s). Below are six relational operators used in specifying the conditions:
...
Let's begin how `subset()` works with vectors, followed by data frames.
...
Let's use the logical NOT to negate the above query, meaning to select pumpkins either lighter than 6 or heavier than 7.
...
It is quite common to randomly sample a subset of items from a vector or rows from a data frame in statistics.
...
Let's say we want to pick 5 samples randomly from `iris`. It takes two steps to achieve that:

**Step 1:** Create a list (vector) of random row indexes by the `sample()` function by specifying the following parameters:

a.  The range of indexes. In this example, the indexes must be in the range from 1 to the last row of `iris`, which can be obtained from `nrow(iris)`.
...
`seq(nrow(iris))` above generates a vector containing 1, 2, 3, ..., 150, as `iris` contains 150 rows.

**Step 2:** Select rows from `iris` according to the random indexes in `rnd_idx`.
...
The indexing method has been discussed in the previous chapter. Check out here: @sec-01-access-rows-columns. A note about `iris[rnd_idx,]` . Do not omit the comma (`,`) after `rnd_idx` inside the square brackets. It means all columns are selected for the output.

### Sort Items

#### Sort Items in a Vector

It can be done using the `sort()` function. By default, the sorting order is ascending (from small to large).
...
Unfortunately, the `sort()` function does not work for the two-dimensional data frame as conveniently as it does for vectors. We have to use a method similar to sampling rows. It takes two steps to sort rows in a certain order. Suppose you want to sort rows in `iris` by `Sepal.Length` in ascending order (small to large).
...
Note that the length of `sorted_idx` is the same as the number of rows in `iris`.
...
Do not overlook the comma (`,`). It is used to retrieve all the rows in the data frame.
...
New columns are always added to the rightmost position, as you can see `Sepal.Ratio`.

Suppose you want to add a new column that records the name of the researcher who collected the sample. If one researcher does it for all samples, you don't have to repeat the same name in a vector. Here is the code:
...
`tidyverse` provides a neater way with no intermediate data frames produced. What is new is the **pipe** concept. You can imagine a pipe is like a factory assembly line, where an upstream workstation passes (pipes) intermediate results to the next workstation for further processing.
...
esoph %>%
  group_by(agegp) %>%
  summarise(n=sum(ncases)+sum(ncontrols))
# The answer below is a common mistake:
#esoph %>%
#  group_by(agegp) %>%
#  summarise(n=n())
#
# This answer will return the number of rows for each age group and they will
# not add up to the total number of subjects 975.
