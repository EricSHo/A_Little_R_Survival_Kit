...
2. `>=`[^02_part1_dw_r-1] greater than or equal to.

3. `<` less than.

4. `<=`[^02_part1_dw_r-1] less than or equal to.

5. `==`[^02_part1_dw_r-1] equal to.

6. `!=`[^02_part1_dw_r-1] not equal to.

[^02_part1_dw_r-1]: No spaces between the two symbols.

If the selection involves multiple conditions, they must be specified in conjunction with relational operators. Here are the three logical operators required for multi-parts conditions:
...
Let say we want to randomly pick 5 samples from `iris`. It takes to two steps to do that:
...
Unfortunately, the `sort()` function does not work for the two-dimensional data frame. We have to use the method similar to sampling rows. It takes two steps to sort rows in a certain order. Suppose you want to sort rows in `iris` by `Sepal.Length` in ascending order (small to large).
...
If a data frame contains many columns, way more than you need for analysis. It is neat to focus on those columns and leave out the rest. Let's suppose you want to focus only on sepal information and species.
...
Suppose you want to add a new column that records the name of the researcher who collected the sample. If it is done by one researcher, you don't have to repeat the same name in a vector. Here is the code:
...
```
The data frame can be tallied by more than one categorical variable. Let's us another built-in dataset `CO2` to illustrate the point.
...
`tidyverse` is an alternative, and importantly, less cryptic way for performing data wrangling of data frames. One design goal of is to name functions using **verbs**, which improves code readability, setting its apart from using unpronounceable symbols, such as `$`, `[`, etc. by base R as shown in the previous section. **However, `tidyverse` works with data frames only.** You still need the base R to process vectors.

Before we dive into `tidyverse`, it is noteworthy to clarify that `tidyverse` is an integrated ecosystem of R packages designed for data science. The data wrangling functions discuss below are actually provided by two packages: `dplyr`, and `tidyr`. As this book is a little book, you can simply assume everything provided by the umbrella package `tidyverse`.

As `tidyverse` is an add-on package, you have to install it manually. But the installation is simple. In RStudio, click the menu option *Tools*, select the first option *Install Packages*. At the *Packages* box, type "tidyverse", and click the "Install" button. If you only need to install it once until after a major R upgrade.
...
The `require()` function has churned out messages telling you what packages (9 of them) are loaded together. 
...
Specify more than one condition. Use a relational operator to join conditions together.
...
To sort rows in descending order, simply mark the sorting column with a minus `-`. E.g.,
...
Specify more than one sorting column is much easier in `tidyverse` than base R. E.g. sort `iris` in descending order of `Sepal.Length` and ascending order of `Petal.Length`.
...
`tidyverse` provides a neater way in which no intermediate data frames are produced. What is new is the **pipe** concept. You can imagine a pipe is like the assembly line of a factory, where an upstream work station passes (pipe) intermediate results to the next work station for further processing.

The pipe symbol is represented by **`%>%`**, no space between them. Let's see how to apply the pipe to the above example without creating the intermediate data frame `tmp_iris`.
