...
To access more than one column and row, pack the row indexes and column names in vectors using the `c(...)` function, which **c**ombines or **c**ollates several items into a vector object (see @sec-vectors). For example, show the `Petal.Length` and `Sepal.Length` of the 123^th^ and 96^th^ rows:
