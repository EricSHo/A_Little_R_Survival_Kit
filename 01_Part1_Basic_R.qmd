...
![Illustration 1.1 R variables and shoeboxes.](images/Illustrate_1-1.png)
...
### Atomic and Composite Variables {#sec-01-atomic-and-composite-variables}
...
![Illustration 1.2 Vectors.](images/Illustrate_1-2.png)
...
The `[2]` means the 2^nd^ element of a vector. We will discuss indexing in more detail below.
...
#### Accessing Rows and Columns of a Data Frame {#sec-01-access-rows-columns}
...
To access more than one column and row, pack the row indexes and column names in vectors using the `c(...)` function, which **c**ombines or **c**ollates several items into a vector object (see @sec-01-vectors). For example, show the `Petal.Length` and `Sepal.Length` of the 123^th^ and 96^th^ rows:

```{r}
iris[c(123,96),c('Petal.Length','Sepal.Length')]
```

If your task is to extract a single column, R provides an alternative way to access a column by its name. The syntax is `<data frame name>$<column name>` [^01_part1_basic_r-2]. For example, `iris$Species`, which is equivalent to `iris[,5]`,
...
Find out more information about the `summary()` function below @sec-summary-function.

It is almost done. There is one nuance. In the current form, `sex` is a character variable, so its summary statistics are a bit strange. Most often, variables like that should be categorical/nominal. Therefore, we need to convert `sex` into a `factor` @sec-factor-levels. Here's the revised code:
...
$$Lower\space whisker = Q1 - 1.5\times{IQR}$$ $$Upper\space whisker = Q3 + 1.5\times{IQR}$$
...
[^01_part1_basic_r-3]: Note that `range()` works for a non-numeric vector, but we will skip such a special operation.
...
```{r echo=F, eval=F}
dude_heights <- c(60, 62, 70, 68, 65)

u <- sum(dude_heights)/length(dude_heights)
print(paste("Sample mean of height =", u))

v <- sum((dude_heights - u)**2)/(length(dude_heights)-1)
print(paste("Sample variance of height =", v))

s <- sqrt(v)
print(paste("Sample SD of height =", round(s,2)))
```
...
```{r echo=F, eval=F}
# watch out the c() inside the factor()
q7a <- factor(c('acidic','basic','hydrophilic','hydrophobic'))
print(q7a)
print(levels(q7a))

q7b <- factor(c('Hypotension' , 'Normal', 'Elevated', 'Hypertension Stage 1', 'Hypertension Stage 2'), ordered = T)
print(q7b)
print(levels(q7b))
```
...
d)  Print the 13^th^ row.

e)  Print the horsepower of the vehicle in the 7^th^ row.
...
```{r echo=F, eval=F}
name <- c('Caleb','Jeshua','Flora')
ph <- c('12345','23456','34567')
height <- c(69, 66, 60)

friends <- data.frame(name, ph, height)
head(friends)
```
...
```{r echo=F, eval=F}
#a
hp_diff <- with(mtcars, max(hp)-min(hp))
print(hp_diff)

#b
with(mtcars, median(hp))

#c
q3 <- with(mtcars, quantile(hp, probs=c(0.75)))
q1 <- with(mtcars, quantile(hp, probs=c(0.25)))
...
upp <- q3 + 1.5*IQR
lwr <- q1 - 1.5*IQR
print(paste("Lower =",lwr, "Upper =",upp))

#e
straight_engine <- with(mtcars, sum(vs))
print(paste("Straight engine =",straight_engine))

v_engine <- nrow(mtcars) - straight_engine
print(paste("V engine =",v_engine))
```
