...
\urlstyle{same}
\hypersetup{
  pdftitle={A\_Little\_R\_Survival\_Kit},
  pdfauthor={Eric S. Ho, Ph.D.},
  hidelinks,
  pdfcreator={LaTeX via pandoc}}


\title{A\_Little\_R\_Survival\_Kit}
...
\chapter{A Little R Survival Kit: Essential Data Analysis using
R}\label{a-little-r-survival-kit-essential-data-analysis-using-r}
...
discusses R objects and operations act on these objects, descriptive
statistical functions, and data wrangling using base R and
\texttt{tidyverse}. Chapter 2 covers on data visualization. Both base R
and \texttt{ggplot2} graphical systems will be discussed. Importantly,
it provides guidelines to readers how to choose the most suitable graph
according to the properties of data. A picture is worth a thousand
words. This book will discuss the purpose, pros and cons of each type of
graph. Part II includes use cases that leverage the topics discussed in
the previous parts.
...
have access to RStudio. There are two options: sign in to the RStudio
cloud service from here:
(https://posit.cloud/plans?utm\_source=Website\&utm\_medium=IDE\_Download),
or install R and RStudio on your own computers. Let me use a car analogy
to illustrate the relationship between R and RStudio. R is the engine
and RStudio is the dashboard. RStudio provides an Interactive
Development Environment (IDE) that provides a user-friendly environment
for you to unleash the power of R. Both of them work on household
Windows and macOS computers. The installation is straightforward; it
involves few double-clicks. Here are the links to download R
...
Part I focuses on the basic R objects and commands, such as atomic and
object variables, descriptive statistical functions, and data wrangling
using base R and \texttt{tidyverse}.

\bookmarksetup{startatroot}

\chapter*{Basic R}\label{basic-r}
\addcontentsline{toc}{chapter}{Basic R}

\markboth{Basic R}{Basic R}
...
This chapter covers common basic R functions used for statistical
analysis. After finishing this chapter, you should be able to:
...
\section*{Rules of Naming Variables}\label{rules-of-naming-variables}
\addcontentsline{toc}{section}{Rules of Naming Variables}

\markright{Rules of Naming Variables}
...
\item
  Subsequent characters can be any combinations of letters, digits
  (0-9), ``\texttt{.}'', or ``\texttt{\_}''.
\end{enumerate}
...
\begin{itemize}
\item
  Variable names are case-sensitive, e.g., \texttt{volume} and
  \texttt{Volume} are different in the eyes of R.
\item
  Give meaningful names to variables, e.g., \texttt{zone2} instead of
  \texttt{z} or even worse \texttt{x}. That will help a bunch in
  understanding the code, even your own code.
\item
  Watch out that some names have been used by R, e.g., \texttt{c}, and
  \texttt{head} are function names. However, R is permissive for people
  to use them as variable names. So, the rule of thumb is to exercise
  self-discipline not to choose R function names as variable names.
\end{itemize}

\section*{R Objects}\label{r-objects}
\addcontentsline{toc}{section}{R Objects}

\markright{R Objects}

\subsection*{Atomic and Object
Variables}\label{atomic-and-object-variables}
\addcontentsline{toc}{subsection}{Atomic and Object Variables}

R classifies variables into two broad categories: \textbf{atomic type} ,
and \textbf{object}. In the former, the value cannot be split further
into smaller R objects. Let us begin with two atomic types:
\texttt{**numeric**}, and \texttt{**character**}.

\subsubsection*{Numeric Variables}\label{numeric-variables}
\addcontentsline{toc}{subsubsection}{Numeric Variables}

The simplest atomic type in R is the \texttt{numeric} data type. That
includes whole numbers, decimal numbers, and numbers represented in
scientific notation (\(6.023\times10^{23}\)). In mathematics, an atomic
variable is a \textbf{scalar}. Here are some examples:
...
Don't be mistaken, the \texttt{e} in the scientific notation refers to
base \texttt{10}, e.g., \texttt{6.023e23} is \(6.023\times10^{23}\).

\texttt{\textless{}-} denotes the assignment operator. Upon execution,
the resultant value of the expression on the right-hand side, if no
error, is stored in the named variable on the left-hand side. In
RStudio, you can see the variable name and its associated value under
the \textbf{Environment} tab, which usually occupies the top right
quadrant of the RStudio desktop.

Note that R supports two equivalent assignment operators. You can use
\texttt{\textless{}-} and \texttt{=} interchangeably. I prefer
\texttt{\textless{}-} to \texttt{=} as \texttt{=} can easily confuse
with the equality checking operator \texttt{==}.

\textbf{Arithmetic Operators}

Arithmetic operators that can apply to numeric variables include
\texttt{+}, \texttt{-}, \texttt{*}, \texttt{/}, \texttt{**}, and
\texttt{\^{}}. The last two operators - \texttt{**} and \texttt{\^{}} -
are exponential; they perform exactly the same function. Here are some
examples:
...
Note that \texttt{pi} is \(\pi\), which is a built-in variable with the
value 3.141593.
...
\addcontentsline{toc}{subsubsection}{Character Variables}

Besides numeric values, R also support operations of non-numeric values,
which is called \texttt{**character**}. To define a character value,
embrace the string of characters with a pair of quotes, either a pair of
...
something else, use the \texttt{class()} function,

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{class}\NormalTok{(course)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{nucleotides}\SpecialCharTok{*}\DecValTok{4}
\end{Highlighting}
\end{Shaded}
...
One commonly used object in R is \textbf{\texttt{vectors}}. It comprises
different components that can be processed collectively as a whole, or
individually. A vector can be viewed pictorially as a wagon train. E.g.,
...
several objects, as its name suggests, into a vector object. Elements
are indexed or numbered from 1 to the total number of elements in the
vector. You can extract a particular element by specifying the index
within a pair of square-brackets. E.g.,
...
\texttt{{[}2{]}} means the 2\textsuperscript{nd} element of a vector. We
will discuss more about indexing below.
...
Noteworthy that all elements held inside a vector must have the same
data type (\textbf{homogeneous}). For instance, \texttt{days\_of\_month}
contains all numeric values, and \texttt{nucleotides} contains all
character values. If you try to combine objects of different data types
into a vector, R will convert them to character type. In the example
below, even the first and third values are entered as numeric, R coerces
them into character type, the common denominator type, \texttt{"1"} and
\texttt{"0"}.
...
data frame is another commonly used object types, which will be
discussed later.

\subsection*{Factors/Categorical Variables}\label{factor-levels}
\addcontentsline{toc}{subsection}{Factors/Categorical Variables}

In R, a categorical variable in statistics is called a \textbf{factor}.
A categorical variable is further classified into \textbf{nominal} or
\textbf{ordinal}. A nominal variable has no intrinsic order. The
\texttt{factor()} function is used to define a categorical variable.
E.g., a sample of colors is stored in a vector, and you want to define
it as a nominal categorical variable.
...
As you can see above, the hypertension levels are ordered and connected
by a less than sign (\texttt{\textless{}}). \#\#\# Data frames

Besides \texttt{vector}, another commonly used object is
\texttt{data\ frame}. Almost. all the datasets we will use for analysis
are in a data frame format. Simply speaking, a data frame is a
\textbf{tabular} or spreadsheet-like object, like an Excel or Google
Sheets. A data frame consists of rows and columns. Each row represents a
sample. Each column represents a variable. Rows are expected to have
equal length, i.e., the same number of columns. If not, R will either
complain or pad the missing columns with a special value \texttt{NA}
(not available). Let's take a look at an R's built-in data frame
\texttt{iris}:
...
You can peek into the bottom five rows by the \texttt{tail()} function.
...
So, we know that \texttt{iris} contains 150 rows (flowers) and five
columns.

\subsubsection*{Accessing Rows and Columns of a Data
Frame}\label{accessing-rows-and-columns-of-a-data-frame}
\addcontentsline{toc}{subsubsection}{Accessing Rows and Columns of a
Data Frame}

Each element in a data frame can be referenced by row and column indexes
like the X-Y Cartesian system. Rows are automatically indexed
(numbered), where the 1st row is index 1, the 2nd row is index 2, and
the index of the last row is the total number of rows, similarly for
columns. But often time, columns are named, such as the columns of
\texttt{iris}; \texttt{Sepal.Width}, \texttt{Sepal.Length}, etc., as
shown in Figure (@ref(fig:Fig1-1)). The two equivalent indexing schemes
can occur in parallel.\footnote{Rows can be named as well. This is left
  as an exercise for the readers.}

The syntax of accessing individual elements is
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{images/Figure_1.1.png}}

}

\caption{Accessing rows and columns of a data frame.}

\end{figure}%

If the column index is ignored, all columns of the specified row(s) are
returned as shown in Figure (@ref(fig:Fig1-1)). For example, show all
columns of the 3\textsuperscript{rd} row by:
...
column(s) are returned (@ref(fig:Fig1-1)). For example,
...
R provides an alternative way to access a single column of a data frame.
The syntax is
\texttt{\textless{}data\ frame\ name\textgreater{}\$\textless{}column\ name\textgreater{}}.
The equivalence of \texttt{iris{[},5{]}} is \texttt{iris\$Species}:
...
Note that the \texttt{\$} method only works for a single column. We will
discuss a way shortly how to access multiple columns in one R command.

Besides the numeric column index, columns can be referenced by names as
shown in Figure (@ref(fig:Fig1-1)). E.g., the \texttt{Petal.Length} of
...
To access more than one column and row, pack the row indices and column
names in vectors by the function \texttt{c(...)}, which
\textbf{c}ombines or \textbf{c}ollates several items into a vector
object. For example, show the \texttt{Petal.Length} and
\texttt{Sepal.Length} of the 123\textsuperscript{th} and
96\textsuperscript{th} rows:
...
Using row index and column names is the basic accessing method. R
provides additional methods to select rows and columns by conditions.
More of this topic will be discussed in \textbf{Chapter 2: Data
Wrangling}.

\subsubsection*{Create a Data Frame}\label{create-a-data-frame}
\addcontentsline{toc}{subsubsection}{Create a Data Frame}
...
\addcontentsline{toc}{subsection}{Summary Functions}

\texttt{length()} shows the number elements in an object. We will use a
sample of pumpkin's weights for illustration.
...
The \texttt{length()} shows the number of weights defined in
\texttt{pumpkins}.
...
To be clear, \texttt{length()} counts the number of elements in an R
object, such as a vector or a data frame. \textbf{If the object is
atomic, the function will return 1.} For example, in the following
example, \texttt{dna} is a character (atomic). The function will return
1, not the number of characters.
...
The \texttt{range()} prints the minimum and maximum values - numeric and
non-numeric - of an object.
...
\addcontentsline{toc}{subsection}{Non-robust Statistics}

Mean, variance, and standard deviation are non-robust statistics, as
they are sensitive to outliers (extreme values).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# the standard deviation}
...
\addcontentsline{toc}{subsection}{Robust Statistics}

Median and quantile are robust statistics, as outliers minimally
...
\textbf{Inter-quantile Region (IQR)}
...
Function}{summary() Function}}\label{summary-function}
\addcontentsline{toc}{subsection}{\texttt{summary()} Function}

A data frame may consists of many variables of different types in
various ranges. Therefore, it makes a lot of sense to take a glimpse of
all variables before analysis. It can be done by the \texttt{summary()}
function to reveal:
...
\addcontentsline{toc}{section}{Utilities}

\markright{Utilities}

\subsection*{Printing and Formatting}\label{printing-and-formatting}
\addcontentsline{toc}{subsection}{Printing and Formatting}
...
The above printout is almost perfect. If you're picky, you don't like
the extra space separating the diameter and the period in the above
message, you can use \texttt{paste()}'s cousin function
\texttt{paste0()}. The difference between the two is that
\texttt{paste0()} will not automatically add an extra space between each
component given to the function. Below is the previous print statement
with \texttt{paste0()}:
...
numbers}{seq() - create a series of numbers}}\label{seq-function}
\addcontentsline{toc}{subsection}{\texttt{seq()} - create a series of
numbers}
...
creating a formal R object. See example below you will understand the
nuance.
...
It set up the context of a data frame so it spares you from using the
\texttt{\$} to access the columns of a data frame. It is commonly used
together with a function applied to one or more columns. E.g., the
median of \texttt{Petal.Width}. It can done in two ways:
...
That's a bare minimum of R scripting required for data analysis. In the
next chapter, we will focus on operations applied to data frames. Before
you leave, it is useful to recap the R functions discussed in this
chapter:
...
\texttt{cat()} & \texttt{summary()} & \texttt{quantile()} & ** \\
\texttt{class()} & \texttt{mean()} & \texttt{IQR()} & \^{} \\
\texttt{factor()} & \texttt{var()} & \texttt{seq()} & == \\
\end{longtable}
...
Data wrangling refers to the way to view, change, and reorganize data in
a data frame. Before we dive into the operations, it is important to
know that there are two distinct schools of data wrangling: base R and
\texttt{tidyverse}, where the latter has contributed substantially in
enhancing the process. We will start with the base R version, followed
by the \texttt{tidyverse}.
...
condition(s). Here are the three relational operators to support
multi-parts conditions:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  ``or'' is denoted by \texttt{\textbar{}}
\item
  ``and'' is denoted by \texttt{\&}
\item
  ``not'' is denoted by \texttt{!}
\end{enumerate}

Note that the operator for checking equality is ``==''; two equal signs
touching each other.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Choose all setosa iris. }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# More than one condition. The relational operator is the logical OR, i.e., a vertical bar |}
...
It is quite common to randomly sample a subset of rows from a data
frame. It takes to two steps to achieve that:

\textbf{Step 1.} Create a list (vector) of random row indices by the
\texttt{sample()} function. Specify the following parameters:

a.) The range of values. For this example, the range is from 1 to the
last row of \texttt{iris}, which is the return of \texttt{nrow(iris)}.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# seq(nrow(iris)) generates 1, 2, 3, ..., 150.}
...
Step 2: select rows from iris according to the indices in
\texttt{rnd\_idx}.
...
It is a two-step process similar to the random sampling of rows from the
previus subsection. Suppose you want to sort \texttt{iris} in
\texttt{Sepal.Length} in ascending order (small to large).

Step 1: Obtain a list of row indices that indicates the sorting order to
the data.
...
Step 2: Rearrange the rows in the data frame according to the sorted row
indices.
...
Note that the above two steps do NOT change the original order of the
rows in the data frame. If you intend to change the row order
permanently, use the assignment operator to update the existing data
frame or create a new data frame to store the sorted rows. See below:
...
position. It can be done using two approaches.
...
Note that if a data frame contains many columns, say \textgreater100, it
is daunting to use this approach. A much better way can be done using
\texttt{dplyr}, so stay tuned.
...
\subsection{Tabulation}\label{base-tabulation}
...
It can tally more than one categorical variable. Let's us another
built-in dataset \texttt{CO2} to illustrate the point.
...
There is an alternate, and importantly, less cryptic way to perform data
wrangling. This new set of functions are offered by the R package
\texttt{dplyr}. One design goal of \texttt{dplyr} is to use
\textbf{verbs} to name these operations, a huge difference from the
unpronounceable symbols, such as \texttt{\$}, \texttt{{[}}, etc. by base
R as shown in the previous section.

Before we dive into \texttt{dplyr}, it is noteworthy to mention that
there is a slew of packages with the aim of improving data cleaning,
data visualization, and loading data in various formats. To spare people
from the question of when to use what package, all these packages are
bundled in a single package called \texttt{tidyverse}. Once you have
activated \texttt{tidyverse}, \texttt{dplyr}, \texttt{ggplot} (for
plotting data), and other packages are available for use. When you call
a function, you don't have to worry if it is from \texttt{dplyr} or
\texttt{tidyr} because the \texttt{tidyverse} community has resolved all
name conflicts. An analogy is like there is only one ``Main Street'' in
\texttt{tidyverse} package; no ambiguity at all. \textbf{The take home
is: just activate \texttt{tidyverse} and you are good to go.}

\texttt{tidyverse} does not come with the base R so you have to install
it manually. But the installation is simple, click the menu option
Tools, select the first option ``install packages'', type ``tidyverse'',
and click the ``Install'' button.

Let's activate \texttt{tidyverse} by using the \texttt{library()}
function. Alternatively, you can use \texttt{require()}; they are
largely doing the same thing. Note that you only need to activate a
package once per session.
...
The \texttt{library()} function has churned out many messages telling
you what packages (9 of them) are loaded together.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# More than one condition. The symbol for the logical AND operator is \&}
...
\subsection{Select Rows Randomly}\label{select-rows-randomly-1}
...
\subsection{Sort Rows in a Data Frame}\label{sort-rows-in-a-data-frame}
...
to change the original data frame or store the changes in a new data
frame. For example, sort \texttt{iris} by \texttt{Sepal.Length}, and
store the sorted result in a new data frame.
...
To sort rows according to the descending order of the sorting column,
simply mark the sorting column with a minus \texttt{-}. E.g.,
...
Specify more than one sorting column is much easier in \texttt{dplyr}
than base R. E.g. sort \texttt{iris} in descending order
\texttt{Sepal.Length} and ascending order of \texttt{Petal.Length}.
...
\subsection{Rearranging columns of a data
frame}\label{rearranging-columns-of-a-data-frame}

Suppose you want to move the \texttt{Species} column to the leftmost
position. You're going to love \texttt{dplyr}, as it can be done easily
by using two functions: \texttt{select()} and \texttt{everything()}.
...
Use the \texttt{mutate()} function add columns to a data frame.
...
Use the \texttt{select()} function as before but put a minus
...
Often, data wrangling is a multi-step process in which many intermediate
steps are involved before producing the final result. E.g., select iris
where its \texttt{Petal.Length} is between 0.5 and 2, and sort the
results in descending order of \texttt{Petal.Length}. As you can see, it
involves filtering, followed by sorting. It can done into two steps via
an intermediate data frame as below:
...
\texttt{tidyverse} provides a neater way - no intermediate data frames -
to do it by the concept of \textbf{pip}. You can imagine it like the
assembly line of a factory, where an upstream work station passes (pipe)
intermediate results to the next work station for further processing.

The pip symbol is represented by \textbf{\texttt{\%\textgreater{}\%}},
no space between them. Let's see how to apply pip to the above example
without creating the intermediate data frame \texttt{tmp\_iris}.
...
By using pip (\texttt{\%\textgreater{}\%}), you can avoid intermediate
variables, making the code cleaner and easily to read.

\subsection{Tabulation and Summarize}\label{tidyverse-tabulation}
...
You can group by more than one column. Let's switch to \texttt{CO2}
...
\item
  pip \texttt{\%\textgreater{}\%}: funnel intermediate result from an
...
capability of \texttt{tidyverse} should consult the most authorative
source: \textbf{R for Data Science} by Hadley Wickham \& Garrett
Grolemund (https://r4ds.had.co.nz/). In the next chapter, you will see
how to plot data. A picture is worth a thousand words. Data
visualization is an indispensible step for data analysis. But before the
...
\texttt{==} & \texttt{NONE} (a keyword) & \texttt{arrange()} & pip
\texttt{\%\textgreater{}\%} \\
...
After laboring yourselves to collect, clean, and analyze the data, you
want to share the findings and engage the audience with insights about
the significance of the study. But most people are not good at
extracting insights from rows and columns of numbers. It is the job of
the researchers to get important messages across to the audience so they
can understand them with as little effort as possible.
...
This chapter discusses the properties of various graphs, what they are
good at and not so good at, and how to plot them in R using two
different graphical systems. After finishing this chapter, you should be
able to:
...
Specially, the types of variables (numeric or categorical) and how many
of them. You will plot graph for:
...
graphs may appear a bit primitive. Some features are hard to be
implemented. Anyway, if plotting graphs using R is new to you, base R is
a great starting point. You will understand the basic characteristics of
different graphs without requiring too much technical details.
...
your concern is robust statistics, a boxplot will do the job.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-1-1.pdf}}

}

\caption{Base R Histogram. Top left: default. Top right: Customized X-Y
labels and the main title. Bottom left: change color of bars and border.
Bottom right: change the bin width (\texttt{breaks=})}

\end{figure}%

For the bottom-right graph, the \texttt{seq()} function
(@ref(seq-function)) is used to generate a list of numbers for the
\texttt{breaks} parameter that represents x-ticks.

Note: \texttt{par(mfrow=c(2,2))} splits the plotting area into a 2x2
grid such that multiple graphs can be grouped

\subsubsection{Boxplot}\label{boxplot}

If the message is robust statistics, such as median and quantiles, or
the presence of outliers, the best graph is a boxplot. The y-axis
represents the numeric variable, and the x-axis has no meaning.
...
\FunctionTok{boxplot}\NormalTok{(iris}\SpecialCharTok{$}\NormalTok{Sepal.Width,}
        ...
        \AttributeTok{main=}\StringTok{"Boxplot of Sepal Width"}\NormalTok{)}
...
\FunctionTok{boxplot}\NormalTok{(iris}\SpecialCharTok{$}\NormalTok{Sepal.Width, }
        ...
        \AttributeTok{xlab=}\StringTok{"Sepal Width"}\NormalTok{, }
        \AttributeTok{main=}\StringTok{"Horizontal Boxplot"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-2-1.pdf}}

}

\caption{Boxplot. Top left: Default, Top right: labels and main title.
Bottom left: color options. Bottom right: \texttt{horizontal\ =\ TRUE}}

\end{figure}%

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{images/Figure_3.3.png}}
...
In practice, values that are greater or less than the upper or lower
whiskers are treated as outliers.

Histogram offers a glimpse of how the data is distributed, displaying
the peak(s), and long tail, if any, that is less obvious in a boxplot.
However, pinpointing the median and outliers on a histogram is
non-trivial. Therefore, it makes sense to combine the strengths of both
plots in a single graph. To be sure the two plots are comparable, make
sure to set the axes in the same range by the \texttt{xlim} and/or
\texttt{ylim=} parameters. Here's an example below:
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-4-1.pdf}}

}

\caption{Stacking up two graphs.}

\end{figure}%

Figure (@ref(fig:Fig3-4))) illustrated the added advantage of combining
graphs. It is clear that the peak in the histogram is almost perfectly
aligned with the median shown in the boxplot, indicating symmetric data
distribution. Additionally, the outliers hardly visible from the
histogram can easily be spotted in the boxplot. This example illustrates
the synergy and additional perspectives created by combining different
graphs.
...
discussed in Chapter 2, the tallying can be done using the
\texttt{table()} function (@ref(base-tabulation)) or
\texttt{group\_by()} followed by \texttt{summarize(n=n())}
(@ref(tidyverse-tabulation)).

\subsubsection{Barplot}\label{barplot}

Barplot is the standard choice for visualizing tallied data. The only
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-5-1.pdf}}

}

\caption{A simple bar plot. Top left: Default. Top right: with tables}
...
right according to the current order of the levels. E.g., the levels of
\texttt{iris\$Species} is:
...
You can change the current order by updating the factor variable by the
\texttt{factor()} function (@ref(factor-levels)). Suppose, the desired
order is \texttt{virginica}, \texttt{setosa}, and \texttt{veriscolor}.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-6-1.pdf}}

}

\caption{Reorder the Bars of a Barplot}

\end{figure}%

\subsubsection{Pie Chart}\label{pie-chart}

Besides a barplot, a pie chart is also a good fit for displaying tallied
data.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-7-1.pdf}}

}

\caption{A simple pie chart. Top left: Default. Top right: with tables}
...
The categorical is usually served as a group identifier that splits the
data into groups so the numerical variable can be compared between
groups. This kind of graph is generally named side-by-side plot. You can
choose a side-by-side boxplot or a side-by-side barplot. For example,
visually compare the robust statistics between different species of
iris.
...
\FunctionTok{boxplot}\NormalTok{(Sepal.Length }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Species,}
        ...
        \AttributeTok{main=}\StringTok{"A Side{-}by{-}Side Boxplot"}\NormalTok{)}

\FunctionTok{boxplot}\NormalTok{(Sepal.Length }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Species,}
        \AttributeTok{data=}\NormalTok{iris, }
        \AttributeTok{horizontal =} \ConstantTok{TRUE}\NormalTok{,}
        \AttributeTok{main=}\StringTok{"A Horizontal Side{-}by{-}Side Boxplot"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-8-1.pdf}}
...
However, there is glitch in the horizontal boxplot. The tick y-labels
collided. It can be fixed by adding \texttt{las=2} parameter to the
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-9-1.pdf}}
...
\subsubsection{Side-by-side Barplot}\label{side-by-side-barplot}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-10-1.pdf}}

}

\caption{Side-by-side Barplot}

\end{figure}%

\subsubsection{Side-by-side Barplot with Error
Bar\^{}*}\label{side-by-side-barplot-with-error-bar}

*This topic is slightly advanced. It is much easier to use
\texttt{ggplot2}. So, feel free to skip this and jump to
@ref(ggplot-error-bar).
...
\texttt{bar\_midpoints}. Use the \texttt{arrows()} function create the
error bars, where each error bar consists of the upper whisker
\((x_{1}, y_{1})\) and lower whisker \((x_{0}, y_{0})\).
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-11-1.pdf}}

}

\caption{Side-by-side Barplot with Error Bars}

\end{figure}%

Once again, if you found this confusing, see the \texttt{ggplot} version
of error bars (Section @ref(ggplot-error-bar)).

\subsubsection{Line Graph with Error
Bar}\label{line-graph-with-error-bar}

A line graph is an alternative to a bar plot that is good at showing a
trend. Here you see how to create a line graph overlaid with solid dots.
...
\FunctionTok{plot}\NormalTok{(mean\_data,}
      ...
      \AttributeTok{xlab=}\StringTok{"Species"}\NormalTok{,}
      \AttributeTok{ylab=}\StringTok{"Average Sepal.Length"}\NormalTok{,}
      \AttributeTok{main=}\StringTok{"Line Plot"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-12-1.pdf}}
...
\item
  ...
  means to plot \textbf{both} dots and line. More options of
  \texttt{type} can be found in the \texttt{help(plot)} documentation.
\end{itemize}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-13-1.pdf}}

}

\caption{Line plot with proper x-tick labels}

\end{figure}%

Here's how to add error bars to the line plot. It is similar to the
barplot above, except that \texttt{x0} and \texttt{x1} should take the
values \texttt{1,2,3}.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-14-1.pdf}}

}

\caption{Line plot with error bars}

\end{figure}%

\subsubsection{Overlapping Histogram\^{}*}\label{overlapping-histogram}

/* The code for creating this plot is a bit convoluted. It is easier to
make it using \texttt{ggplot2}. Feel free to skip to
@ref(ggplot-overlapping-histogram).

Suppose you are interested in comparing the distribution of sepal
lengths among the three iris species. Here are the steps:
...
\item
  Create a histogram for a data subset from step 1, which becomes the
  template of the final graph. Additionally, color is used to
  distinguish one species from the other.
\item
  Overlay the histograms for other data subsets on top of the template.
\end{enumerate}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-15-1.pdf}}

}

\caption{Overlapping histogram}

\end{figure}%

\subsection{Contingency Table}\label{contingency-table}

If a dataset comprises two categorical variables. A two-dimensional
contingency table, or a 2D table, depicts the number of samples present
in different combinations of categories of the two categorical
variables. The distribution of the counts can reveal the dependency of
the two categorical variables. Let's use another built-in dataset
\texttt{penguins} for illustration. It contains two categorical
variables: \texttt{species} and \texttt{island}. Suppose you are
interested to explore how geographical location affects the distribution
of penguins.
...
you can see not all three species can be found in each island. If
...
Two popular graphs are used to visualize counts from a contingency
table: a side-by-side barplot, and mosaic plot.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-16-1.pdf}}

}

\caption{Visualization of a 2D Contingency Table. A. A side-by-side
barplot. B. Mosaic plot.}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-16-2.pdf}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-16-3.pdf}}
...
If you want to gain a gut feeling about the relationship between two
numeric variables, a scatter plot is the best choice. Note that the
relationship can be manifold, ranging from linear to cyclical. So,
visualizing how the data points are distributed in a 2D plan is a quick
and easy way in gaining intuition of the two variables. Importantly, it
should be done before performing any statistical analysis, e.g., linear
regression. Here is how to create a scatter plot that shows the
relationship between sepal width and sepal length. Note that each point
\((x_{i},y_{i})\) is a pair of values from one sample.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-17-1.pdf}}

}

\caption{Visualizing the relationship between two numeric variables.}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-18-1.pdf}}

}

\caption{Color data points by species.}

\end{figure}%

If your aim is to explore linear relationship between the two variables,
it can be emphasized by plotting a regression line for the two variables
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-19-1.pdf}}

}

\caption{Add a regression line}

\end{figure}%

\subsection{Multi-panel Scatter Plot}\label{multi-panel-scatter-plot}

A multi-panel scatter plot offers a glimpse of the relationship between
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-20-1.pdf}}

}

\caption{A Multi-panel Scatter Plot}
...
(e.g., multi-panel scatter plot), while others are complicated (e.g.,
barplot with error bars). To compare the pros and cons of the two
graphical systems, let's continue our journey to \texttt{ggplot}.

\section{\texorpdfstring{\texttt{ggplot2}}{ggplot2}}\label{ggplot2}

\texttt{ggplot2} is bundled in \texttt{tidyverse}. So, you have to
activate \texttt{tidyverse} before using \texttt{ggplot2}. Here, the
...
To use \texttt{ggplot2}, load it into R.
...
\subsection{\texorpdfstring{Grammer of
\texttt{ggplot2}}{Grammer of ggplot2}}\label{grammer-of-ggplot2}

The two '\texttt{g}'s of \texttt{ggplot2} stand for \textbf{Grammar of
Graphics}. The graphical widgets are arranged in layers that provides
the flexibility for decorating a graph by adding layers to it. The
\textbf{data} layer forms the foundation of a graph. The
\textbf{Aesthetics} defines the association between data variable(s) and
the axes. \textbf{Geometries} are the types of graphs, e.g., line plot,
histogram, etc. \textbf{Facets} split a graph into multi-panels.
\textbf{Statistics} defines how to aggregate data, e.g., in proportion
or take the face value in a barplot. \textbf{Coordinates} supports an
X-Y Cartesian coordinate system, and a polar coordinate system. (See the
Pie Chart section below) \textbf{Themes} offers different color schemes,
font type, font size, etc.

\begin{figure}[H]

{\centering \includegraphics[width=0.5\linewidth,height=0.5\textheight]{images/Figure_3-21-ggplot-anatomy.png}

}

\caption{Anatomy of \texttt{ggplot2}}
...
Let's redraw the histogram in Figure (@ref(fig:Fig3-1))) using
\texttt{ggplot2}. The first line is the data layer that specifies the
data source, and it must be a data frame (or tibble), e.g.,
\texttt{iris}. The aesthetic option designates the x-axis for
\texttt{Sepal.Width}. The second line is the geometry of the graph. For
a histogram, the function name is \texttt{geom\_histogram()}. Here is
the bare minimum of a \texttt{ggplot2} histogram:
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-22-1.pdf}}

}

\caption{\texttt{ggplot2} Histogram.}

\end{figure}%

It is always a good idea to give the graph a title at the top so people
what the key message is. Equally important is the x- and y-labels. This
task can be done by the \texttt{labs()} function. By default, the title
is left-justified. To center it, see the last line and the comment above
it.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-23-1.pdf}}

}

\caption{\texttt{ggplot2} Histogram - specify your own labels and
title.}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-24-1.pdf}}

}

\caption{\texttt{ggplot2} Histogram - choose your own color}

\end{figure}%

The bin width of the histogram is calculated automatically by
\texttt{ggplot2} according to the range of x. To overwrite it by the
\texttt{breaks} parameter.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-25-1.pdf}}
...
plotting area instead of displaying in a 2x2 grid as Figure
(@ref(fig:Fig3-1))). It is because \texttt{ggolot2} does not work with
...
install it the very first time.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-26-1.pdf}}

}

\caption{\texttt{ggplot2} Grid System by \texttt{patchwork}. Top left:
Default (bare minimum). Top right: with labels and title. Bottom left:
change the color. Bottom right: change the number of bins.}

\end{figure}%

\subsubsection{Boxplot}\label{boxplot-1}

For an anatomy of a boxplot, reference the Figure (@ref(fig:Fig3-3))).
The geometry function of a boxplot is \texttt{geom\_boxplot()}. Unlike a
histogram, the numeric variable is mapped to the y-axis for a vertical
boxplot. If you want a horizontal boxplot, map the numeric variable to
...
\NormalTok{p2 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data=}\NormalTok{iris, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{y=}\NormalTok{Sepal.Width)) }\SpecialCharTok{+}
        ...
        \FunctionTok{labs}\NormalTok{(}\AttributeTok{y=}\StringTok{"Sepal Width"}\NormalTok{, }\AttributeTok{title =} \StringTok{"Boxplot of Sepal Width"}\NormalTok{) }\SpecialCharTok{+}
        \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}
...
\CommentTok{\# Horizontal boxplot}
\NormalTok{p4 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data=}\NormalTok{iris, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{Sepal.Width)) }\SpecialCharTok{+}
        \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
        \FunctionTok{labs}\NormalTok{(}\AttributeTok{x=}\StringTok{"Sepal Width"}\NormalTok{, }\AttributeTok{title =} \StringTok{"Horizontal Boxplot"}\NormalTok{) }\SpecialCharTok{+}
        \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-27-1.pdf}}

}

\caption{Boxplot using \texttt{ggplot2}. Top left: default (bare
minimum). Top right: with y-label and title. Bottom left: change color.
Bottom right: horizontal boxplot.}
...
Here we will show how to make a barplot and a piechart using
\texttt{ggplot2}.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-28-1.pdf}}

}

\caption{Barplot using \texttt{ggplot2}. Top left: default (bare
minimum).}
...
Although \texttt{ggplots} is portrayed as an improved method to compose
graphs based upon its grammatical architecture, it does not always make
plotting as simple as you might think, like the histograms and barplots
illustrated above. A pie chart is one of those graphs in which
\texttt{geom\_pie()} does not exist! Plotting it is not as
straightforward as the base R \texttt{pie()} function, as shown in
Figure (@ref(fig:Fig3-7))). Despite that, it is worth to see how a pie
chart can be plotted using foundational building-block functions. To
give you a heads-up, \texttt{ggplot2} views a pie chart as a twisted
barplot. There is an internal logic to justify such a view but we will
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-29-1.pdf}}

}

\caption{Pie chart or a donut chart}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-30-1.pdf}}

}

\caption{A stacked barplot}

\end{figure}%

Now, use your imagination to picture that the stacked bar is bent into a
ring. Does it look like the pie chart in Figure (@ref(fig:Fig3-30)))? To
way to ``bend'' it in \texttt{ggplot2} is to change the x-y Cartesian
...
{\centering \includegraphics[width=0.5\linewidth,height=0.5\textheight]{images/Figure_3-25-polar-coords.png}

}

\caption{Polar Coordinate. (generated by Gemini)}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-32-1.pdf}}

}

\caption{A Piechart by changing to the polar coordinate system.}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-33-1.pdf}}

}

\caption{Piechart using \texttt{ggplot2}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-34-1.pdf}}

}

\caption{Piechart using \texttt{ggplot2} without no background.}
...
\subsubsection{Side-by-side Boxplot}\label{side-by-side-boxplot-1}
...
\NormalTok{p1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data=}\NormalTok{iris, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{Species, }\AttributeTok{y=}\NormalTok{Sepal.Length)) }\SpecialCharTok{+}
        \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
        \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"A Side{-}by{-}Side Boxplot"}\NormalTok{) }\SpecialCharTok{+}
        \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}
...
\NormalTok{p2 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data=}\NormalTok{iris, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{Sepal.Length, }\AttributeTok{y=}\NormalTok{Species)) }\SpecialCharTok{+}
        \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
        \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"A Horizontal Side{-}by{-}Side Boxplot"}\NormalTok{) }\SpecialCharTok{+}
        \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-35-1.pdf}}
...
\subsubsection{Side-by-side Barplot}\label{side-by-side-barplot-1}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-36-1.pdf}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-37-1.pdf}}

}

\caption{Side-by-side barplot with error bar}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-38-1.pdf}}

}

\caption{Line plot with error bar}

\end{figure}%

\subsubsection{Overlapping
Histogram}\label{ggplot-overlapping-histogram}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-39-1.pdf}}

}

\caption{Side-by-side Histogram}

\end{figure}%

\subsection{Contingency Table}\label{contingency-table-1}

We will use the \texttt{penguins} data set to demonstrate how to create
a side-by-side bar plot of the counts, and a mosaic plot.
...
As said above, \texttt{ggplot2} only works with data frames. Therefore,
you need to convert the contingency table into a data frame with column
names. During the conversion process, the R function will pair up each
species with an island and create a new column that stores the count. It
means that the resultant data frame will consists of RxC rows, where R
and C are the number of rows and columns, respectively. A 3x3
contingency table like yours, will be converted to a data frame
consisting of 9 rows as shown below:
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-40-1.pdf}}
...
Here, you are going to visualize the relationship between two numeric
variables using a scatter plot. As said above, the revealed relationship
can be linear and non-linear. Visualizing how data points are
distributed on a scatter plot is the most intuitive way to understand
...
\FunctionTok{ggplot}\NormalTok{(iris, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{Sepal.Width, }\AttributeTok{y=}\NormalTok{Sepal.Length)) }\SpecialCharTok{+}
  ...
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{x=}\StringTok{"Sepal Width"}\NormalTok{, }\AttributeTok{y=}\StringTok{"Sepal Length"}\NormalTok{, }\AttributeTok{title =} \StringTok{"Scatter Plot"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme}\NormalTok{(}\AttributeTok{plot.title =} \FunctionTok{element\_text}\NormalTok{(}\AttributeTok{hjust =} \FloatTok{0.5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-42-1.pdf}}
...
above to determine what the slope and intercept are. Instead, it can be
done by adding the \texttt{geom\_smooth()}. You tell
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-43-1.pdf}}

}

\caption{A scatter plot overlaid with a regression line.}
...
\subsection{Multi-panel Scatter Plot}\label{multi-panel-scatter-plot-1}

Often times before multiple linear regression, it is a good idea to
visualize the linear relationship between the response variable and the
independent variables pairwisely. Or, the linear relationship between
independent variables, as it may cause the collinearity issue in which
independent variables are linearly correlated. A multi-panel scatter
plot can do this. However, \texttt{ggplot2} does not have a geometry
function for it. The simplest way is to use the \texttt{ggpairs()}
function provided by the \texttt{GGally} package, which requires
installation, if you have not done so.
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{03-Data-Visualization_files/figure-pdf/Fig3-44-1.pdf}}
...
{\centering \includegraphics[width=0.5\linewidth,height=0.5\textheight]{images/pch_symbols.png}

}

\caption{\texttt{pch} values}

\end{figure}%

\textbf{Color}

R supports a large number of colors and palettes. Here is a great
resource to see what are available and their names:
...
\pandocbounded{\includegraphics[keepaspectratio]{04_Dive_Deeper_files/figure-pdf/unnamed-chunk-4-1.pdf}}
