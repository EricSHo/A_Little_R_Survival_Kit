...
equals a specified value. & t.test(FlightDelays\$Delay, mu = 12) &
Section~\ref{sec-04-one-sample-t-test} \\
Two-sample t-test & Test whether or not the means of two groups are
equal. & t.test(Delay \textasciitilde{} Carrier, data=FlightDelays) &
Section~\ref{sec-04-two-sample-t-test} \\
One-way ANOVA & Test whether or not the means of three or more groups
are equal. & aov\_results \textless- aov(Delay \textasciitilde{}
Destination, data=FlightDelays) & Section~\ref{sec-04-one-way-ANOVA} \\
...
categorical variables & with(FlightDelays, chisq.test(Carrier,
Delayed30, correct=TRUE)) & Section~\ref{sec-04-x2-test-independence} \\
Simple Linear Regression & It examines the linear relationship between
the predictor and the response. & lm(Sepal.Width \textasciitilde{}
Sepal.Length, data=setosa\_iris) & Section~\ref{sec-04-SLR} \\
Multiple Linear Regression & It examines the linear relationship between
multiple predictors and the response. & lm(Sepal.Width \textasciitilde{}
Sepal.Length + Petal.Length, data=setosa\_iris) &
