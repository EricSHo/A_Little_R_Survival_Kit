...
the museum. The statistics you hear about in the news and research are
produced by computers, the theme of this little book.

This book uses R. You may ask why not use a spreadsheet, such as
Microsoft Excel or Google Sheets. While they are excellent tools - easy
to use - they have significant shortcomings. In particular, they lack
automation, as they operate through point-clicking, drag-and-drop, etc.,
hampering reproducibility.

This book is written for readers who have scant experience in computer
programming or scripting. R is a big language. At times, it is dizzying
to navigate a vast amount of materials in the ocean of the Internet.
Therefore, this book intends to provide readers with the bare minimum of
R to start analyzing data. Specifically, I want them to focus on using R
fast and get things done rather than being bogged down by the technical
details and cryptic syntax of the R language. Although most people think
a book like this is about R programming, I prefer to call it
\textbf{scripting}. In a nutshell, programming encapsulates algorithms -
a step-by-step procedure to tackle a problem like a cooking recipe.
While it is an important topic, here we focus on the what (the expected
results) instead of the how (algorithms). You tell the R functions: here
is the data and the parameters; please process them and return the
results.
...
discusses R objects and operations that act on these objects, including
...
to swim. You don't just listen to instructions. You've got to jump into
the water and get wet! To try out the examples and exercises, you need
to use RStudio. There are two options to access RStudio: a cloud service
(Posit Cloud) and installing RStudio on your own computer. For the first
option, no software installation is required. All you need is a Web
Browser with an internet connection. The first time, you can sign up for
an account and choose a plan from
...
regardless of what computer you use, as long as you log in to the same
account.

Alternatively, if you prefer better control of RStudio, you can install
two software packages, R and RStudio, on your own computers. However, it
becomes your responsibility to maintain and update them in the future.
Why do you need to install two programs instead of just RStudio? In a
car analogy, R is like the engine, and RStudio is like the dashboard.
...
If your computer is a Chromebook, Posit Cloud is preferred, as
installing R and RStudio on a Chromebook is less straightforward. For
Windows and Mac, the installation is simple; it only takes a few
...
Frame}\label{sec-01-access-rows-columns}
...
The \texttt{subset()} function selects rows from a data frame if they
satisfy the condition(s). Below are six relational operators used in
...
Let's begin how \texttt{subset()} works with vectors, followed by data
frames.
...
Let's use the logical NOT to negate the above query, meaning to select
pumpkins either lighter than 6 or heavier than 7.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{subset}\NormalTok{(pumpkins, }\SpecialCharTok{!}\NormalTok{(pumpkins}\SpecialCharTok{\textgreater{}=}\DecValTok{6} \SpecialCharTok{\&}\NormalTok{ pumpkins}\SpecialCharTok{\textless{}=}\DecValTok{7}\NormalTok{))}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
[1]  7.55  4.45 10.14  3.52  3.76  7.62
\end{verbatim}

The above query can be rewritten using the logical OR as follows:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{subset}\NormalTok{(pumpkins, pumpkins}\SpecialCharTok{\textless{}}\DecValTok{6} \SpecialCharTok{|}\NormalTok{ pumpkins}\SpecialCharTok{\textgreater{}}\DecValTok{7}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
rows from a data frame in statistics.
...
takes two steps to achieve that:

\textbf{Step 1:} Create a list (vector) of random row indexes by the
\texttt{sample()} function by specifying the following parameters:

\begin{enumerate}
\def\labelenumi{\alph{enumi}.}
\item
  The range of indexes. In this example, the indexes must be in the
  range from 1 to the last row of \texttt{iris}, which can be obtained
  from \texttt{nrow(iris)}.
\item
...
\texttt{seq(nrow(iris))} above generates a vector containing 1, 2, 3,
\ldots, 150, as \texttt{iris} contains 150 rows.

\textbf{Step 2:} Select rows from \texttt{iris} according to the random
indexes in \texttt{rnd\_idx}.
...
The indexing method has been discussed in the previous chapter. Check
out here: Section~\ref{sec-01-access-rows-columns}. A note about
\texttt{iris{[}rnd\_idx,{]}} . Do not omit the comma (\texttt{,}) after
\texttt{rnd\_idx} inside the square brackets. It means all columns are
...
It can be done using the \texttt{sort()} function. By default, the
sorting order is ascending (from small to large).
...
two-dimensional data frame as conveniently as it does for vectors. We
have to use a method similar to sampling rows. It takes two steps to
sort rows in a certain order. Suppose you want to sort rows in
\texttt{iris} by \texttt{Sepal.Length} in ascending order (small to
large).
...
Note that the length of \texttt{sorted\_idx} is the same as the number
of rows in \texttt{iris}.
...
Do not overlook the comma (\texttt{,}). It is used to retrieve all the
rows in the data frame.

An alternative approach is to make use of column indexes as below:
...
New columns are always added to the rightmost position, as you can see
\texttt{Sepal.Ratio}.

Suppose you want to add a new column that records the name of the
researcher who collected the sample. If one researcher does it for all
samples, you don't have to repeat the same name in a vector. Here is the
code:
